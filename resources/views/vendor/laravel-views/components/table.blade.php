...
<table class="min-w-full">
  ...
  <thead class="border border-t border-gray-200 bg-gray-100 text-xs leading-4 font-semibold uppercase tracking-wider text-center">
    <tr>
    ...
    @foreach ($items as $item)
      <tr class="border-b border-gray-200 text-sm" wire:key="{{ $item->getKey() }}">
        ...
        @foreach ($view->row($item) as $column)
          <td class="px-3 py-2 whitespace-no-wrap border border-t border-gray-200">
            {!! $column !!}
          </td>
        @endforeach
